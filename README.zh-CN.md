...
- **支持调试** - 可选择使用 VS Code 调试器运行已保存文件，从而命中断点
- 自动检测 Node.js 版本：
...
你可以使用 VS Code 调试器运行已保存的文件，从而命中断点。

1. 打开 VS Code 设置，启用 `quickRunJsTs.enableDebug`。
2. 在已保存的文件上点击 **Quick Run JS/TS: Run**。
3. 文件将以调试模式启动，原始文件中的断点会生效。
...
| `quickRunJsTs.enableDebug`       | `false`         | 使用 VS Code 调试器运行已保存文件，从而命中断点                                       |
